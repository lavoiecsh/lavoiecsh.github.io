...
int main() {
  ...
  for (auto p : passwords) {
    int lc = 0;
    if (p.password[p.min-1] == p.letter) ++lc;
    if (p.password[p.max-1] == p.letter) ++lc;
    debug() << pp(p.password) pp(p.letter) pp(p.min) pp(p.max) pp(p.password[p.min-1]) pp(p.password[p.max-1]) pp(lc);
    if (lc == 1) ++count;
  }
  cout << count << endl;

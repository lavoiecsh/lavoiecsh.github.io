...
vector<string> plan;
int count(int row, int col, char c) {
  int x = 0;
  for (int y = 1; row - y >= 0 && col - y >= 0; ++y) {
    if (plan[row-y][col-y] == c) ++x;
    if (plan[row-y][col-y] != '.') break;
  }
  for (int y = 1; row - y >= 0; ++y) {
    if (plan[row-y][col] == c) ++x;
    if (plan[row-y][col] != '.') break;
  }
  for (int y = 1; row - y >= 0 && col + y < plan[0].length(); ++y) {
    if (plan[row-y][col+y] == c) ++x;
    if (plan[row-y][col+y] != '.') break;
  }
  for (int y = 1; col - y >= 0; ++y) {
    if (plan[row][col-y] == c) ++x;
    if (plan[row][col-y] != '.') break;
  }
  for (int y = 1; col + y < plan[0].length(); ++y) {
    if (plan[row][col+y] == c) ++x;
    if (plan[row][col+y] != '.') break;
  }
  for (int y = 1; row + y < plan.size() && col - y >= 0; ++y) {
    if (plan[row+y][col-y] == c) ++x;
    if (plan[row+y][col-y] != '.') break;
  }
  for (int y = 1; row + y < plan.size(); ++y) {
    if (plan[row+y][col] == c) ++x;
    if (plan[row+y][col] != '.') break;
  }
  for (int y = 1; row + y < plan.size() && col + y < plan[0].length(); ++y) {
    if (plan[row+y][col+y] == c) ++x;
    if (plan[row+y][col+y] != '.') break;
  }
  return x;

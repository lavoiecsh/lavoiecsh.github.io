...
long simple_evaluate(string expression) {
  ...
  if (mult != expression.npos) {
    return simple_evaluate(expression.substr(0, mult-1)) * simple_evaluate(expression.substr(mult+1));
  }
  auto plus = expression.find('+');
  if (plus != expression.npos) {
    return simple_evaluate(expression.substr(0, plus-1)) + simple_evaluate(expression.substr(plus+1));
  }
